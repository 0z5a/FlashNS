"""G1/G2 complete first activation VJP and coordinate wgrad screening.

Both arms share the complete native VJP arithmetic and coordinate reducer.
G2 removes the first bar_Z materialization; forward GEMMs and torch tail remain identical.
"""
...
MODES = ("coordinate", "fused")


class FirstVjpWgradGraph:
    def __init__(self, problem, flat, native, mode):
        ...
        self.metadata = dict(self.step.metadata, first_vjp_mode=mode,
...
def run_screening(problem, native, args, report, save, locked):
    ...
    flat = parameters(531902).to("cuda")
    weight_hash = digest(flat)
    engines = {mode: FirstVjpWgradGraph(problem, flat, native, mode) for mode in MODES}
    ...
    pairs = (("A/A", "coordinate", "coordinate"), ("coordinate/fused", "coordinate", "fused"))
    ...
    result = {"status": "running", "scope": "complete compact loss/parameter-gradient Graph replay",
              "matched_control": "complete native VJP materialized then the same coordinate reducer",
    ...
    for index, plan in enumerate(schedule):
        ...
        save()
        print(f"first VJP/wgrad block {index+1}/{len(schedule)} {plan['comparison']} {plan['pattern']}", flush=True)
    locked()
    ...
    result["decision"] = (
        ...
        and all(result["summary"]["coordinate/fused"][metric]["bootstrap_95pct_interval"][0] > 1.01 for metric in METRICS)
...
def main():
    ...
    parser.add_argument("--order-seed", type=int, default=92717)
    ...
    report = {"completed": False, "screening": None,
              "contract": {"candidate": "coordinate_activation_wgrad_G2", "atol": 2e-12, "rtol": 2e-11,
                           "weight_seed": 531902, "order_seed": args.order_seed,
    ...
    try:
        ...
        if (not preflight.get("completed") or not preflight.get("preflight_passed")
                ...
                or preflight.get("contract", {}).get("candidate") != "coordinate_activation_wgrad_G2"
            ...
            raise RuntimeError("full first-VJP/wgrad preflight for exact source, binary and GPU required")
