"""Matched first-affine/activation fusion screening after full GPU preflight.

Both arms use identical scalar coordinate-affine arithmetic and stable a1.
The comparison isolates the first Z materialization/activation-launch boundary.
"""
...
class FirstGraph:
    def __init__(self, problem, flat, native, mode):
        ...
        self.metadata = dict(self.step.metadata, first_mode=mode,
...
def run_screening(problem, native, args, report, save, locked):
    ...
    engines = {mode: FirstGraph(problem, flat, native, mode) for mode in MODES}
    ...
    pairs = (("A/A", "separate", "separate"), ("separate/fused", "separate", "fused"))
    ...
    result = {"status": "running", "scope": "complete compact loss/parameter-gradient Graph replay",
              "matched_control": "same coordinate coefficient helper, separate native stable activation",
    ...
    for index, plan in enumerate(schedule):
        ...
        save()
        print(f"first fusion block {index+1}/{len(schedule)} {plan['comparison']} {plan['pattern']}", flush=True)
    locked()
    ...
    result["decision"] = (
        ...
        and all(result["summary"]["separate/fused"][metric]["bootstrap_95pct_interval"][0] > 1.01 for metric in METRICS)
...
def main():
    ...
    parser.add_argument("--order-seed", type=int, default=89243)
    ...
    report = {"completed": False, "screening": None,
              "contract": {"candidate": "coordinate_affine_activation", "atol": 2e-12, "rtol": 2e-11,
                           "weight_seed": 361902, "order_seed": args.order_seed,
    ...
    try:
        ...
        if (not preflight.get("completed") or not preflight.get("preflight_passed")
                ...
                or preflight.get("contract", {}).get("candidate") != "coordinate_affine_activation"
            ...
            raise RuntimeError("full first-fusion preflight for exact source, binary and GPU required")
