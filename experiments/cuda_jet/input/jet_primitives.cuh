...
FLASHNS_JET_HD void tanh_fwd_2d3(const double* z, double* h) {
...
FLASHNS_JET_HD void tanh_vjp_2d3(const double* h, const double* bh, double* bz) {
...
FLASHNS_JET_HD void tanh_fwd_3d3(const double* z, double* h) {
  const double t = ::tanh(z[0]);
  const double a1 = 1.0 - t*t;
  const double a2 = -t*a1;
  const double a3 = a1*(t*t - 1.0/3.0);
...
FLASHNS_JET_HD void tanh_vjp_3d3(const double* h, const double* bh, double* bz) {
  const double g0 = 1.0 - h[0]*h[0];
  const double g1 = -(2.0 * h[0] * h[1]);
  const double g2 = -(2.0 * h[0] * h[2]);
