...
if [[ $# != 2 || ! "$2" =~ ^GPU-[0-9a-fA-F-]{36}$ ]]; then
  echo 'Usage: FLASHNS_PYTHON=python bash scripts/run_sm120_first_vjp_wgrad.sh NEW_OUTPUT GPU_UUID' >&2
  exit 2
fi
...
for flashns_tool in memcheck racecheck initcheck synccheck; do
  timeout 300 "$flashns_sanitizer" --tool "$flashns_tool" --error-exitcode 86 \
    "$flashns_python" experiments/pinn_v8/first_vjp_wgrad_preflight.py --build "$flashns_output/build" \
...
PY
timeout 900 "$flashns_python" experiments/pinn_v8/first_vjp_wgrad_preflight.py --build "$flashns_output/build" \
  --output "$flashns_output/first-vjp-wgrad-preflight.json" > "$flashns_output/first-vjp-wgrad-preflight.log" 2>&1
timeout 900 "$flashns_python" experiments/pinn_v8/first_vjp_wgrad_screening.py --build "$flashns_output/build" \
  --preflight "$flashns_output/first-vjp-wgrad-preflight.json" --output "$flashns_output/first-vjp-wgrad-screening.json" \
  > "$flashns_output/first-vjp-wgrad-screening.log" 2>&1
echo "Completed SM120 first VJP/wgrad correctness and screening: $flashns_output"
