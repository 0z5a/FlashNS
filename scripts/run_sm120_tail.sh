...
if [[ $# != 2 || ! "$2" =~ ^GPU-[0-9a-fA-F-]{36}$ ]]; then
  echo 'Usage: FLASHNS_PYTHON=python bash scripts/run_sm120_tail.sh NEW_OUTPUT GPU_UUID' >&2
  exit 2
fi
...
for flashns_tool in memcheck racecheck initcheck synccheck; do
  timeout 300 "$flashns_sanitizer" --tool "$flashns_tool" --error-exitcode 86 \
    "$flashns_python" experiments/pinn_v8/tail_sm120.py --build "$flashns_output/build" \
...
PY
timeout 900 "$flashns_python" experiments/pinn_v8/tail_sm120.py --build "$flashns_output/build" \
  --output "$flashns_output/tail-screening.json" > "$flashns_output/tail-screening.log" 2>&1
echo "Completed SM120 correctness and screening: $flashns_output"
